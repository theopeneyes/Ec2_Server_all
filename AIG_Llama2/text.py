...
def getText(id):
    ...
    if file_type == "mp4" or file_type == "mp3" or file_type == "wav" or file_type=="avi" or file_type=="flv" or file_type=="mov" or file_type=="webm" or file_type=="wmv" or file_type=="mkv":
...
# We will check file path, parse it and we will get Text_pdf which contains each page of document
    ...
    if file_type == "pdf":
        ...
        if n_pages <= 10:
            ...
            print("Length of Characters: ",total)
            if total < 200 :
                model_error = {"message":"Please upload a file with More then 200 characters"}
            else:
                text = "Complete Text"
        else:
            model_error={"message":"Please upload a file with less than 10 pages"}
        ...
        for i, content in enumerate(xhtml_data.find_all('div', attrs={'class': 'page'})):
            ...
            for i in Text_pdf:
                total = len(i.strip().split()) + total
            if total < 0 :
                model_error = {"message":"Please upload a file with Content"}
            else:
                text = "Complete Text"
        else:
            model_error={"message":"Please upload a file with More then 200 characters"}
    ...
    elif file_type == "mp4" or file_type == "mp3" or file_type == "wav" or file_type == "wav" or file_type=="avi" or file_type=="flv" or file_type=="mov" or file_type=="webm" or file_type=="wmv" or file_type=="mkv":
        Text_pdf = []
        total = 0
        ...
        for key, text_is in enumerate(texts):
            if key == 0:
                if len((text_is.description).split()) > 200:
                    text_all = text_is.description
                    text = text_all.encode('utf-8')
                    # Decode the bytes to string after encoding
                    text = text.decode('utf-8')
                    text = text.replace("\r", '')
                    print("Content fetched...",text)
                else:
                    
                    model_error={"message":"Please upload an Image with More then 200 characters"}
                    final,text=None,None
                    
    
                    return  file_type,final,text,model_error,type_of_question,_id
        ...
        if len(existing_files) > 0:
            ...
            with open(pdf_file_path, 'rb') as f:
                ...
                try: 
                    ...
                    # Handle HTTP errors, for example, if a file with the same name already exists
                    print('ERROR: Unable to upload PDF file. %s' % str(e))
