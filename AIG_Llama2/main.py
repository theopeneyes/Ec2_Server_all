...
def AIG(user_id):
    ...
    # request=requests.get("https://generate-questions.devbyopeneyes.com/api/getFileData/" + user_id)
    request=requests.get("https://aig.theopeneyes.com/api/getFileData/" + user_id)
    resp = request.json()
    file_type = (resp["data"]["file_type"])
    result = None
    if file_type != "mp4":
        file_type, final,text_content,model_error,type_of_question, _id= getText(user_id)
        if model_error != None:
            ...
            try:
                recommended_questions,additional_questions,error_ques_list = getAllGeneratedQuestions(final, type_of_question,file_type)
                user_required_questions = recommended_questions
                other_questions = additional_questions
                error_ques_list_ = error_ques_list
            except:
                status_url = "https://aig.theopeneyes.com/api/updateFileStatus"
                # status_url = "https://generate-questions.devbyopeneyes.com/api/updateFileStatus"
                headers = {'Content-Type':'application/json','Accept':'application/json'}
